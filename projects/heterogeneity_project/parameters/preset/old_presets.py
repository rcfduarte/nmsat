...
"""
This file needs to be maintained to be able to read data stored using the previous version of the code
"""
...
def set_kernel_defaults(default_set=0, run_type='local', data_label='', project_label=None, **system_pars):
	...
	if default_set == 1:
		...
		raise IOError("default_set not defined for this project")
...
def randomized_neuron_parameters(heterogeneous=True):
	"""
...
def connectivity_parameters(default_set=0):
	"""
	Retrieve stored connectivity parameters dictionaries
	:return:
	"""
	if default_set == 0:
		global_AMPA = 0.06
		...
		epsilon = {'EE': 0.168, 'EE_copy': 0.168,
		           'I1E': 0.575, 'I1E_copy': 0.575,
		           'I2E': 0.242, 'I2E_copy': 0.242,
		           'EI1': 0.6, 'EI1_copy': 0.6,
		           'EI2': 0.465, 'EI2_copy': 0.465,
		           'I1I1': 0.55, 'I1I1_copy': 0.55,
		           'I2I1': 0.241, 'I2I1_copy': 0.241,
		           'I1I2': 0.379, 'I1I2_copy': 0.379,
		           'I2I2': 0.381, 'I2I2_copy': 0.381}
		delays = {'EE': 1.2, 'EE_copy': 1.2,
		          'I1E': 1., 'I1E_copy': 1.,
		          'I2E': 1., 'I2E_copy': 1.,
		          'EI1': 0.5, 'EI1_copy': 0.5,
		          'EI2': 0.8, 'EI2_copy': 0.8,
		          'I1I1': 0.5, 'I1I1_copy': 0.5,
		          'I2I1': 0.5, 'I2I1_copy': 0.5,
		          'I1I2': 0.7, 'I1I2_copy': 0.7,
		          'I2I2': 0.8, 'I2I2_copy': 0.8}
		weights = {'EE': global_AMPA * wEE, 'EE_copy': global_NMDA * wEE,
		           'I1E': global_AMPA * wIE, 'I1E_copy': global_NMDA * wIE,
		           'I2E': global_AMPA * wIE, 'I2E_copy': global_NMDA * wIE,
		           'EI1': global_GABAa * (gamma_E * wEE), 'EI1_copy': global_GABAb * (gamma_E * wEE),
		           'EI2': global_GABAa * (gamma_E * wEE), 'EI2_copy': global_GABAb * (gamma_E * wEE),
		           'I1I1': global_GABAa * (gamma_I1 * wIE), 'I1I1_copy': global_GABAb * (gamma_I1 * wIE),
		           'I2I1': global_GABAa * (gamma_I2 * wIE), 'I2I1_copy': global_GABAb * (gamma_I2 * wIE),
		           'I1I2': global_GABAa * (gamma_I1 * wIE), 'I1I2_copy': global_GABAb * (gamma_I1 * wIE),
		           'I2I2': global_GABAa * (gamma_I2 * wIE), 'I2I2_copy': global_GABAb * (gamma_I2 * wIE)}

	return epsilon, delays, weights
...
def set_connection_defaults(default_set=0, syn_pars=None, neuron_pars=None):
	...
	connection_pars = {}
	if default_set == 1:
		print "\nLoading Default Connection Set 1 - No topology"
		#############################################################################################################
		# SYNAPSE Parameters
		# ============================================================================================================
	...
	if default_set == 0:
		...
		encoding_pars = {
			...
			'connectivity': {
				...
				'topology_dependent': [False for _ in range(len(syn_pars.target_population_names))],
				'conn_specs': syn_pars.conn_specs,
				'syn_specs': syn_pars.syn_specs,
