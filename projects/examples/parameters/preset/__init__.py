...
def set_neuron_defaults(default_set=0):
    ...
    """
    if default_set == 0:
        print("\nLoading Default Neuron Set - iaf_psc_exp, fixed voltage threshold, fixed absolute refractory "
        ...
        print("\nLoading Default Neuron Set - aeif_cond_exp, fixed voltage threshold, fixed absolute refractory "
...
def set_decoding_defaults(output_resolution=1., to_memory=True, **decoder_pars):
    ...
    if len(dec_pars.readout_algorithms) == N:
        ...
        readouts = [{'N': N, 'labels': dec_pars.readouts, 'algorithm': [
            dec_pars.readout_algorithms[0]]} for _ in range(n_decoders)]
...
def add_background_noise(encoding_pars, noise_pars):
    ...
    encoding_pars.connectivity.synapse_name.extend([label + '_syn' for _ in range(len(connections))])
