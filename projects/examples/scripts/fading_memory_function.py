...
from modules.visualization import set_global_rcParams, InputPlots, plot_target_out, plot_fmf
...
# ======================================================================================================================
params_file = '../parameters/fading_memory.py'
...
time_axis = np.arange(analysis_interval[0], analysis_interval[1], parameter_set.input_pars.noise.resolution)
...
# Determine baseline
baseline = InputNoise(parameter_set.input_pars.noise, stop_time=analysis_interval[1])
baseline.generate()
baseline_out = baseline.noise_signal.time_slice(t_start=analysis_interval[0], t_stop=analysis_interval[1]).as_array()\
...
for ctr, n_pop in enumerate(list(itertools.chain(*[net.merged_populations,
	...
	if n_pop.decoding_layer is not None:
		results['memory'].update({n_pop.name: {}})
		...
		for idx_state, n_state in enumerate(n_pop.decoding_layer.state_variables):
			results['memory'][n_pop.name].update({n_state + str(idx_state): {}})
			...
			# determine baseline reconstruction error
			results['memory'].update({'baseline': calculate_error(baseline_out, input_signal, display=display)})

			for readout in readouts:
				print "Readout {0}".format(readout.name)
				readout.set_index()
				readout.train(state_matrix, input_signal, index=readout.index, display=display)
				readout.measure_stability(display=display)

				output, tgt = readout.test(state_matrix, input_signal, index=readout.index, display=display)

				results['memory'][n_pop.name][n_state + str(idx_state)].update(
					{readout.name: calculate_error(output.T, tgt, display=display)})

			if plot:
				# fig1 = pl.figure()

				keys = np.sort(results['memory'][n_pop.name][n_state + str(idx_state)].keys())
				fmf = [results['memory'][n_pop.name][n_state + str(idx_state)][x]['fmf'] for x in keys]
				plot_fmf(time_axis, fmf, ax=None, label=n_state + str(idx_state), display=display, save=paths[
					                                                                                        'figures'] + paths['label'])
