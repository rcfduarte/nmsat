...
def run(parameter_set, plot=False, display=False, save=True, debug=False, online=True):
	"""
	Simulate stimulus driven network - presenting input in separate datasets (transient, unique, train and test) and
	performing analysis after each stimulus set
				...
				if not empty(labels) and not empty(state_matrix):
					...
					for readout in readouts:
						results['performance'][n_pop.name][var].update(
							{readout.name: readout_test(readout, state_matrix,
