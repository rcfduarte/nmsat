...
def plot_histogram(tmpa, nbins, norm=True, mark_mean=False, mark_median=False, ax=None, color='b', display=True, \
    ...
    tmpa = tmpa[tmpa != 0]
    if np.any(tmpa[np.isnan(tmpa)]):
        print("Removing NaN")
        tmpa = tmpa[~np.isnan(tmpa)]
    if np.any(tmpa[np.isinf(tmpa)]):
        print("Removing inf")
        tmpa = tmpa[~np.isinf(tmpa)]

    n = 0
    bins = 0
    if norm and list(tmpa):
        weights = np.ones_like(tmpa) / float(len(tmpa))
...
def plot_histograms(ax_list, data_list, n_bins, args_list, colors=None, cmap='hsv'):
    """
...
def plot_state_matrix(state_mat, stim_labels=None, ax=None, label='', display=True, save=False):
    """
...
def scatter_projections(state, label_sequence, cmap, ax=None, display=False):
    """
    ...
    for color, index, lab in zip(ccs, unique_labels, unique_labels):
        ...
        tmp = ax.scatter(state[np.where(np.array(list(itertools.chain(
            label_sequence))) == index)[0], 0], state[np.where(
            np.array(list(itertools.chain(label_sequence))) == index)[0], 1], state[np.where(
            np.array(list(itertools.chain(label_sequence))) == index)[0], 2], s=20, c=color, alpha=0.8, label=lab)
        scatters.append(tmp)
