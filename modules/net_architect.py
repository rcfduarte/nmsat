...
def extract_delays_matrix(src_gids, tgets_gids, progress=True):
    """
    Extract the synaptic delay matrix referring to connections from src_gids to tgets_gids
    :param src_gids: list or tuple of gids of source neurons
     :param tgets_gids: list or tuple of gids of target neurons
     :param progress: display progress bar
     :return: len(src_gids) x len(tgets_gids) weight matrix
...
class Population(object):
    ...
    def __init__(self, pop_set):
        ...
        self.seq_id = None  # [int] sequence if of population according to the order defined in the parameters file
        self.children = None  # [list] populations (objects) this merged one is composed of
        self.is_merged = False  # [bool] whether this population is a merged population (e.g. E1I1)
    ...
    def load_activity(self, initializer, t_start=None, t_stop=None):
        """
        ...
        if isinstance(initializer, basestring) or isinstance(initializer, list):
            ...
            # analog signal recorder
                ...
                if t_start is not None and t_stop is not None:
                    # TODO this can be merged into a oneliner! add comments...
                    idx1 = np.where(times >= t_start)[0]
                    idx2 = np.where(times <= t_stop)[0]
                    idxx = np.intersect1d(idx1, idx2)
                    times = times[idxx]
    ...
    def connect_decoders(self, decoding_pars):
        ...
        :param decoding_pars: [dict] or [ParameterSet]
        :return:
        """
...
class Network(object):
    ...
    def __init__(self, net_pars_set):
        ...
        def create_populations(net_pars_set_):
            populations = [[] for _ in range(net_pars_set_.n_populations)]
            ...
            for n in range(net_pars_set_.n_populations):
                if isinstance(net_pars_set_.pop_names[n], list):
                    ...
                    populations[n] = []
                    for nn in range(len(net_pars_set_.pop_names[n])):
                        # create sub-population dictionary
                        subpop_dict = {k: v[n][nn] for k, v in net_pars_set_.iteritems() if k not in not_allowed_keys
                        ...
                        subpop_dict.update({k: v[n] for k, v in net_pars_set_.iteritems() if k not in not_allowed_keys
                                            and (not isinstance(v[n], list))})
                        neuron_dict = subpop_dict['neuron_pars']
                        nest.CopyModel(subpop_dict['neuron_pars']['model'], net_pars_set_.pop_names[n][nn])
                        nest.SetDefaults(net_pars_set_.pop_names[n][nn],
                                         parameters.extract_nestvalid_dict(neuron_dict, param_type='neuron'))
                        if net_pars_set_.topology[n][nn]:
                            tp_dict = subpop_dict['topology_dict']
                            tp_dict.update({'elements': net_pars_set_.pop_names[n][nn]})
                            ...
                            gids = nest.Create(net_pars_set_.pop_names[n][nn], n=int(net_pars_set_.n_neurons[n][nn]))
                    ...
                    pop_dict = {k: v[n] for k, v in net_pars_set_.iteritems() if k not in not_allowed_keys}
                    neuron_dict = net_pars_set_.neuron_pars[n]

                    # create neuron model named after the population
                    nest.CopyModel(net_pars_set_.neuron_pars[n]['model'], net_pars_set_.pop_names[n])
                    # set default parameters
                    nest.SetDefaults(net_pars_set_.pop_names[n], parameters.extract_nestvalid_dict(neuron_dict,
                                                                                                   param_type='neuron'))

                    if net_pars_set_.topology[n]:
                        tp_dict = pop_dict['topology_dict']
                        tp_dict.update({'elements': net_pars_set_.pop_names[n]})
                        layer = tp.CreateLayer(tp_dict)
                        gids = nest.GetLeaves(layer)[0]
                        ...
                        # create population
                        gids = nest.Create(net_pars_set_.pop_names[n], n=int(net_pars_set_.n_neurons[n]))
    ...
    def get_population_by_name(self, name):
        """
        Returns a population given its name. Raises ValueError if not found.

        :param name: [str] name of Population (e.g. 'E1')
        :return: Population object
        """
        for pop in list(itertools.chain(*[self.merged_populations, self.populations])):
            if pop.name == name:
                return pop
        raise ValueError("Population named '{0}' not found!".format(name))
    ...
    def merge_subpopulations(self, sub_populations, name='', merge_activity=False, store=True):
        ...
        :param merge_activity: [bool] whether to merge previously extracted and currently stored spiking/analog activity
        :param store: [bool] if true, append the newly created population to the list of merged populations
        ...
        # store sequence ids of sub-populations which make up the new merged one
        new_population.children = [p.seq_id for p in sub_populations]
    ...
    # TODO rewrite all output %s to .format()
                ...
                if self.record_analogs[n]:
                    if isinstance(self.record_analogs[n], bool):
                        ...
                        for nnn in range(int(self.record_analogs[n])):
                            ...
                            dev_gid = self.populations[n].record_analog(dev_pars, ids=ids,
                                                                        record=dev_dict['record_from'])
                            self.device_gids[n].append(dev_gid)
    ...
    # TODO maybe break up into several functions?
    def connect_populations(self, connect_pars_set, progress=False, batch_mode=False):
        """
        ...
        :param progress:
        :param batch_mode: if connection parameters allow it, connect one presynaptic to all its postsynaptic neurons
        ...
        for n in range(connect_pars_set.n_synapse_types):
            ...
            if 'copy' in synapse_name:  # re-connect the same neurons...
                ...
                if (connect_pars_set.pre_computedW[n] is not None) and (not connect_pars_set.topology_dependent[n]):
                    ...
                    if self.verify_consistent_dimensions(w, src_gids, tget_gids):
                        for preSyn_matidx, preSyn_gid in enumerate(src_gids):
                            ...
                            if not isinstance(connect_pars_set.delay_dist[n], dict) and isinstance(
                                    connect_pars_set.delay_dist[n], list):
                            ...
                            if not batch_mode:
                                # we can do this more efficiently in batch mode if parameters allow it, but in this
                                # case iterate through each connection one-by-one
                                for idx, tget in enumerate(postSyn_gids):
                                    update_dict = {'model': synapse_name, 'weight': weights[idx], 'delay': delays[idx]}
                                    syn_dict = parameters.copy_dict(connect_pars_set.syn_specs[n], update_dict)
                                    nest.Connect([preSyn_gid], [tget], syn_spec=syn_dict)

                            # TODO is it not always possible to do batch mode? Weight and delay matrices are computed
                            # TODO above, separately, so this shouldn't affect
                            elif len(postSyn_gids):
                                # do this in batch mode, calling nest.Connect only once, if there's only one synapse
                                # model. Weight and delay matrices have been computed above. Reshaping is necessary
                                # for NEST..
                                update_dict = {'model': synapse_name,
                ...
                # 3) if no precomputed weights, but topology in populations and topological connections
                    ...
                    assert nest.is_sequence_of_gids(src_gids) and len(src_gids) == 1, \
                        "Source ids are not topology layer"
                    assert nest.is_sequence_of_gids(tget_gids) and len(tget_gids) == 1, \
                        "Target ids are not topology layer"
    ...
    def flush_records(self, decoders=False):
        ...
        :param decoders: [bool] if true, delete data stored in the decoding layer as well
        :return:
        """
                    ...
                    if pop.attached_devices:
                        ...
                        for nnn in pop.attached_devices:
                            if nest.GetStatus(nnn)[0]['to_memory']:
                                ...
                                pop.load_activity(nnn, t_start=t_start, t_stop=t_stop)
                            elif nest.GetStatus(nnn)[0]['to_file']:
                                # initialize load_activity with file paths
                                pop.load_activity(list(nest.GetStatus(nnn)[0]['filenames']),
                ...
                if p.attached_devices:
                    ...
                    for pop in p.attached_devices:
                        if nest.GetStatus(pop)[0]['to_memory']:
                            # initialize load_activity with device gid
                            p.load_activity(pop, t_start=t_start, t_stop=t_stop)
                        elif nest.GetStatus(pop)[0]['to_file']:
                            p.load_activity(list(nest.GetStatus(pop)[0]['filenames']), t_start=t_start, t_stop=t_stop)
    ...
    def merge_population_activity(self, start=0., stop=1000., save=False):
        ...
        :return:
        """
        if not signals.empty(self.merged_populations):
            for m_pop in self.merged_populations:
                assert m_pop.is_merged

                m_spiking_activity = signals.SpikeList([], [], start, stop, m_pop.size)
                m_analog_activity = []

                # iterate over each parent population (ids) in the merged one and collect spiking and analog activities
                for child_seq_id in m_pop.children:
                    # only add spiking activity if it's not empty
                    if not self.populations[child_seq_id].spiking_activity.empty():
                        child_spiking_activity = self.populations[child_seq_id].spiking_activity
                    ...
                    # add analog activity
                    m_analog_activity.append(self.populations[child_seq_id].analog_activity)
    ...
    def extract_synaptic_delays(self, src_gids=None, tget_gids=None):
        """
        Read and store the synaptic delays for all the connected populations, or just for
        the provided sources and targets.
    ...
    def connect_decoders(self, decoding_pars):
        ...
        :param: decoding_pars: [ParameterSet or dict]
        :return:
        """
        if isinstance(decoding_pars, dict):
            decoding_pars = parameters.ParameterSet(decoding_pars)
